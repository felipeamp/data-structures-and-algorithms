...
public:
  explicit Exact(const std::vector<T> &vec) {
    ...
    for (size_t start = 0; start < vec.size(); ++start) {
      ...
      for (size_t end = 1; end <= vec.size(); ++end) {
...
//       if (rmq_low_blocks_.find(cartesian_number) == rmq_low_blocks_.end()) {
//         std::vector<T> block(block_start_it, block_end_it);
//         rmq_low_blocks_[cartesian_number] = Exact<T>(block);
//       }
//     }
...
//     rmq_high_blocks_ = Sparse<T>(high_blocks);
//   }
...
//     size_t end_block_num = end / block_size_;
//     if (end_block_num <= start_block_num + 1) {
//       return rmq_low_blocks_[blocks_cartesian_numbers_[start_block_num]].query(
...
//         end_block_num * block_size_
//         + rmq_low_blocks_[blocks_cartesian_numbers_[end_block_num]].query(
//           0, end - end_block_num * block_size_));
...
//       } else if (first_block_min <= high_blocks_min
//                  && first_block_min <= last_block_min) {
...
// private:
//   size_t block_size_;
//   Sparse<T> rmq_high_blocks_;
//   std::vector<unsigned long long> blocks_cartesian_numbers_;
//   std::unordered_map<unsigned long long, Exact<T>> rmq_low_blocks_;
//   std::vector<T> vec_;

//   unsigned long long get_cartesian_number_(
//       const std::vector<T>::iterator start_it,
//       const std::vector<T>::iterator end_it) {
//     std::stack<T> s;
//     unsigned long long cartesian_number = 0;
//     for (auto it = start_it; it < end_it; ++it) {
//       T curr_value = *curr_it;
