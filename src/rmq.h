...
public:
  explicit Sparse(const std::vector<T> &vec) {
    ...
    log2_.push_back(0);
    size_t log = 0;
    for (size_t slice_size = 1; slice_size < vec.size(); slice_size *= 2, ++log) {
      std::fill_n(log2_.end(), slice_size, log);
    }

    this->block_min_ = std::vector<std::unordered_map<size_t, size_t>>;
    block_min_.reserve(vec_.size());
    for (size_t index = 0; index < vec_.size(); ++index) {
      block_min_.push_back(std::unordered_map<size_t, size_t>());
      block_min_[index].reserve(log2_[vec_.size()]);
  ...
  size_t query(size_t start, size_t end) {
    size_t block_size = log2_[end - start];
